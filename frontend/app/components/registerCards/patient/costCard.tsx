...
export default function CostCard({ pageNumber }: { pageNumber: number }) {
  ...
  return (
    <Card className="w-xl max-w-screen border-none pl-8 pr-4">
      ...
      <CardContent className="grid gap-2 w-fit">
        <RegisterButton label="ja" selected={cost} setSelected={setCost} />
        <RegisterButton label="nein" selected={cost} setSelected={setCost} />
      </CardContent>
    </Card>
  );
}
